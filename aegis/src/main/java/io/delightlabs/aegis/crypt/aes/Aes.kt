...
object Aes {
    ...
    fun encrypt(plainText: ByteArray, key: ByteArray): ByteArray {
        val hashedKey = blake2b(32, key)
        val ivKey = blake2b(16, key)

        val hmacText = ivKey + plainText

        val cipher = Cipher.getInstance("AES/CBC/PKCS5Padding")
        cipher.init(Cipher.ENCRYPT_MODE, SecretKeySpec(hashedKey, "AES"), IvParameterSpec(ivKey))
        val paddedPlainText = padPKCS7(hmacText, cipher.blockSize)

        val encrypted = cipher.doFinal(paddedPlainText)

        val sliceEncrypted = encrypted.dropLast(16).toByteArray()
    ...
    fun decrypt(cipherText: ByteArray, key: ByteArray): ByteArray {
        ...
        decipher.init(Cipher.DECRYPT_MODE, SecretKeySpec(hashedKey, "AES"), IvParameterSpec(ivKey))

        val data = decipher.update(decoded)
        val decryptedText = data + decipher.doFinal()

        if (!(ivKey.contentEquals(decryptedText.take(16).toByteArray()))) {
            throw IllegalArgumentException("Wrong key")
        }

        return decryptedText.slice(IntRange(16, decryptedText.size-1)).toByteArray()
    }

    fun padPKCS7(plainText: ByteArray, blockSize: Int): ByteArray {
        val padding = blockSize - (plainText.size % blockSize)
        val padText = ByteArray(padding) { padding.toByte() }
        return plainText + padText
    }

   fun trimPKCS5(text: ByteArray): ByteArray {
